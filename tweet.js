...
var img = n("*").getUrlValues("pfp");
var txt = n("*").getUrlValues("txt");
var tag = n("*").getUrlValues("tag");
var naym = n("*").getUrlValues("name");
var attach = n("*").getUrlValues("att");

var rt = n("*").getUrlValues("rt");
var like = n("*").getUrlValues("like");
var qu = n("*").getUrlValues("qu");

var ats = n("*").getUrlValues("ats");
var hash = n("*").getUrlValues("hash");

var time = n("*").getUrlValues("time");
var date = n("*").getUrlValues("date");

if (img == null || txt == null || tag == null || naym == null || rt == null || like == null || qu == null || time == null || date == null) {
    document.getElementById("overlay").style.display = "block";
} else {
    document.getElementById("pic").setAttribute("src", img);
    if (ats != null) {
        document.getElementById("coo").innerHTML = `<h1 style="color: lightblue; float: left;">` + "@" + ats + "</h1>";
        document.getElementById("coo").innerHTML = document.getElementById("coo").innerHTML  + `<h1 style="float: left; right: -5px; position: relative;">` + " " + txt + "</h1>";
    } else {
        document.getElementById("coo").innerHTML = "<h1>" + txt + "</h1>";
    }
    if (hash != null) {
        document.getElementById("coo").innerHTML = document.getElementById("coo").innerHTML + ` <h1 style="color: lightblue; right: -15px; position: relative;">` + " #" + hash + "</h1><br><br>";
    }
    ...
    document.getElementById("name").textContent = naym;
    document.getElementById("retwee").textContent = rt;
    document.getElementById("liek").textContent = like;
    document.getElementById("qu").textContent = qu;


    document.getElementById("titlee").textContent = naym + " on Twitter: \"" + txt + "\"";
}

if (attach != null) {
    n("[content]").add("<br><br>")
    n("[content]").add(`<img style="max-width: 500px; border-radius: 8px;" id="at" src="${attach}">`);
}
